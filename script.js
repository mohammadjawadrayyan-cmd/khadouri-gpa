...
function gradeLabel(score){
  ...
  if (score >= 90) return `${t("grade_excellent")} (Excellent)`;
  if (score >= 80) return `${t("grade_verygood")} (Very Good)`;
  if (score >= 70) return `${t("grade_good")} (Good)`;
  if (score >= 60) return `${t("grade_acceptable")} (Pass)`;
  return `${t("grade_fail")} (Fail)`;
}
...
function validateCourses(courses){
  ...
  const filledRows = $$(".row", coursesEl).filter(r=>{
...
function calcCumulative(){
  ...
  const sem = calcSemester(); // ✅ احسب من جديد دائمًا
  if (!sem) return null;
...
function renderCumulative(res){
  ...
  parts.push(`${lang==="ar"?"التقدير":"Rating"}: ${res.label || "—"}`);
  if (res.delta !== null && Number.isFinite(res.delta)){
    parts.push(`${lang==="ar"?"تحسّن":"Improvement"}: ${res.delta >= 0 ? "+" : ""}${res.delta.toFixed(2)}`);
  }
...
function saveAndOpenPrint(){
  ...
  const cum = calcCumulative(); // ممكن يكون null
  ...
  const payload = {
    ...
    cumulative: (cum && Number.isFinite(cum.newAvg)) ? {
      ...
      delta: (cum.delta === null ? null : cum.delta)
    } : null
  };

  localStorage.setItem(LS_PAYLOAD, JSON.stringify(payload));
  window.open(`print.html?ts=${Date.now()}`, "_blank", "noopener,noreferrer");
}
